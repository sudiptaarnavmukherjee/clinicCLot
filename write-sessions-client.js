...
const DEFAULT_SLOT: DaySlot = { start_time: "09:00", end_time: "13:00", max_appointments: "30" };
...
export default function SessionsClient({
  ...
  const [form, setForm] = useState<{ doctor_id: string; daySlots: Record<number, DaySlot>; notes: string }>({
    doctor_id: doctors[0]?.id || "",
    daySlots: {},
    notes: "",
  });

  // Map: doctor_id -> Set of already-scheduled day_of_week values
  const existingDaysPerDoctor = useMemo(() => {
    const map: Record<string, Set<number>> = {};
    for (const rs of recurringSessions) {
      if (!map[rs.doctor_id]) map[rs.doctor_id] = new Set();
      map[rs.doctor_id].add(rs.day_of_week);
    }
    return map;
  }, [recurringSessions]);

  // Group recurring sessions by doctor for display
  const doctorGroups = useMemo(() => {
    const groups: Record<string, { doctor: RecurringSessionWithDoctor["doctors"]; rows: RecurringSessionWithDoctor[] }> = {};
    for (const rs of recurringSessions) {
      if (!groups[rs.doctor_id]) groups[rs.doctor_id] = { doctor: rs.doctors, rows: [] };
      groups[rs.doctor_id].rows.push(rs);
    }
    for (const g of Object.values(groups)) {
      g.rows.sort((a, b) => a.day_of_week - b.day_of_week);
    }
    return Object.values(groups);
  }, [recurringSessions]);

  // Already-scheduled days for the selected doctor (greyed out in create modal)
  const takenDays = editingRecurring
    ? new Set<number>()
    : (existingDaysPerDoctor[form.doctor_id] ?? new Set<number>());

  function openCreate() {
    setEditingRecurring(null);
    setForm({ doctor_id: doctors[0]?.id || "", daySlots: {}, notes: "" });
    setShowForm(true);
  }

  function openAddDays(doctorId: string) {
    setEditingRecurring(null);
    setForm({ doctor_id: doctorId, daySlots: {}, notes: "" });
    setShowForm(true);
  }
  ...
  function toggleDay(day: number) {
    setForm((prev) => {
      ...
      return { ...prev, daySlots: newSlots };
    });
  }

  function updateDaySlot(day: number, field: keyof DaySlot, value: string) {
    setForm((prev) => ({
      ...prev,
      daySlots: {
        ...prev.daySlots,
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const slots = Object.entries(form.daySlots);
    if (slots.length === 0) { toast.error("Select at least one day"); return; }

    for (const [, slot] of slots) {
      if (slot.end_time <= slot.start_time) { toast.error("End time must be after start time"); return; }
      const max = parseInt(slot.max_appointments);
      if (isNaN(max) || max < 1 || max > 500) { toast.error("Max appointments must be 1-500"); return; }
    }
    ...
    try {
      ...
      if (editingRecurring) {
        // Editing a single day row
        const slot = Object.values(form.daySlots)[0];
        const payload = {
          start_time: slot.start_time + ":00",
          end_time: slot.end_time + ":00",
          max_appointments: parseInt(slot.max_appointments),
        ...
        // Creating: upsert one row per selected day
        const rows = slots.map(([day, slot]) => ({
        ...
        const { data, error } = await supabase
          .from("recurring_sessions")
          .upsert(rows, { onConflict: "pharmacy_id,doctor_id,day_of_week" })
          .select("*, doctors(id, name, specialty)");
        if (error) { toast.error(error.message); return; }
        const saved = data || [];
        setRecurringSessions((prev) => {
          const updated = prev.map((r) => {
            const match = saved.find((d) => d.id === r.id);
            return match ? match : r;
          });
          const brandNew = saved.filter((d) => !prev.find((r) => r.id === d.id));
          return [...updated, ...brandNew];
        });
        toast.success("Schedule saved! Today's session will be created if today is included.");
      }
  ...
  async function deleteRecurring(rs: RecurringSessionWithDoctor) {
    if (!confirm("Delete " + DAYS[rs.day_of_week] + " schedule for Dr. " + (rs.doctors?.name ?? "") + "?")) return;
    const supabase = createClient();
  ...
  function DoctorScheduleCard({ doctor, rows }: {
    doctor: RecurringSessionWithDoctor["doctors"];
    rows: RecurringSessionWithDoctor[];
  }) {
    return (
      <div className="bg-white rounded-2xl border border-border overflow-hidden hover:shadow-md transition-all">
        <div className="h-1 w-full bg-gradient-to-r from-blue-400 to-teal-500" />
        <div className="p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="flex items-center gap-2">
                <p className="font-bold text-foreground text-sm">Dr. {doctor?.name ?? "Unknown"}</p>
          ...
          <div className="space-y-2 mb-4">
            {rows.map((rs) => (
              <div key={rs.id} className={"flex items-center gap-2 p-2.5 rounded-xl border transition-all " + (rs.is_active ? "bg-muted/40 border-transparent" : "bg-gray-50 border-gray-100 opacity-60")}>
                <span className={"text-[11px] font-bold px-2 py-1 rounded-lg w-10 text-center flex-shrink-0 " + (rs.is_active ? "bg-blue-600 text-white" : "bg-gray-300 text-gray-600")}>
                  {DAYS[rs.day_of_week]}
                </span>
                ...
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <button onClick={() => toggleRecurringActive(rs)} title={rs.is_active ? "Pause" : "Activate"}
                    className={"p-1.5 rounded-lg border transition-colors " + (rs.is_active ? "border-amber-200 text-amber-600 hover:bg-amber-50" : "border-green-200 text-green-600 hover:bg-green-50")}>
                    {rs.is_active ? <PauseCircle className="w-3 h-3" /> : <PlayCircle className="w-3 h-3" />}
                  </button>
                  <button onClick={() => deleteRecurring(rs)} title="Remove this day" className="p-1.5 rounded-lg border border-border text-muted-foreground hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-colors">
          ...
          <button onClick={() => openAddDays(rows[0].doctor_id)}
            className="w-full flex items-center justify-center gap-1.5 py-2 border-2 border-dashed border-border rounded-xl text-xs font-semibold text-muted-foreground hover:border-blue-300 hover:text-blue-600 transition-colors">
            <Plus className="w-3 h-3" />Add More Days
          </button>
        </div>
      </div>
    );
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Sessions auto-generate each day. Set different times per day of the week.</p>
          </div>
        ...
        {doctorGroups.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {doctorGroups.map((g) => (
              <DoctorScheduleCard key={g.rows[0].doctor_id} doctor={g.doctor} rows={g.rows} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-12 text-center bg-blue-50/50 rounded-3xl border-2 border-dashed border-blue-200">
            ...
            <p className="text-sm text-muted-foreground mb-5 max-w-sm">
              Set different days &amp; different times for each doctor. E.g. Mon 9am, Wed 11am, Sun 10pm. Sessions appear automatically.
            </p>
            <button onClick={openCreate} disabled={doctors.length === 0}
      ...
      {todaySessions.length === 0 && recurringSessions.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 text-center">
          ...
          <p className="text-amber-700 text-xs mt-1">
            {DAY_FULL[new Date().getDay()]} is not in any active recurring schedule.
          </p>
        </div>
      )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="font-bold text-lg text-white">
                    {editingRecurring ? "Edit Day Schedule" : "New Recurring Schedule"}
                  </h2>
                  <p className="text-sm text-blue-100 mt-0.5">
                    {editingRecurring
                      ? "Update time and capacity for " + DAYS[editingRecurring.day_of_week]
                      : "Pick days — set different times for each"}
                  </p>
                </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, doctor_id: e.target.value, daySlots: {} }))}
                  disabled={!!editingRecurring}
                  required
                  className="w-full h-11 px-4 rounded-xl border border-input text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary bg-white disabled:opacity-60 disabled:cursor-not-allowed">
              ...
              {!editingRecurring && (
                <div>
                  ...
                  {takenDays.size > 0 && (
                    <p className="text-[11px] text-muted-foreground">Gray days are already scheduled. Click Edit on a row to change times.</p>
                  )}
                </div>
              )}

              {/* Per-day time inputs */}
              {Object.keys(form.daySlots).length > 0 && (
                <div className="space-y-3">
                  <label className="block text-sm font-medium text-foreground">
                    {editingRecurring ? "Time & Capacity" : "Time & Capacity per Day"}
                  </label>
                  {(Object.entries(form.daySlots) as [string, DaySlot][])
                    .sort(([a], [b]) => Number(a) - Number(b))
                    .map(([day, slot]) => (
                      <div key={day} className="bg-muted/50 rounded-xl p-3 border border-border/50">
                        <div className="flex items-center gap-2 mb-2.5">
                          <span className="text-xs font-bold bg-blue-600 text-white px-2 py-1 rounded-lg w-10 text-center">{DAYS[Number(day)]}</span>
                          <span className="text-xs text-muted-foreground font-medium">{DAY_FULL[Number(day)]}</span>
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            <label className="block text-[11px] text-muted-foreground mb-1">Start time</label>
                            <input type="time" value={slot.start_time}
                              onChange={(e) => updateDaySlot(Number(day), "start_time", e.target.value)}
                              required
                              className="w-full h-9 px-2 rounded-lg border border-input text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
                          </div>
                          <div>
                            <label className="block text-[11px] text-muted-foreground mb-1">End time</label>
                            <input type="time" value={slot.end_time}
                              onChange={(e) => updateDaySlot(Number(day), "end_time", e.target.value)}
                              required
                              className="w-full h-9 px-2 rounded-lg border border-input text-xs focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
                          </div>
                          <div>
                            <label className="block text-[11px] text-muted-foreground mb-1">Max patients</label>
                            <input type="number" value={slot.max_appointments}
                              onChange={(e) => updateDaySlot(Number(day), "max_appointments", e.target.value)}
              ...
              {!editingRecurring && Object.keys(form.daySlots).length === 0 && (
                <div className="text-center py-4 text-sm text-muted-foreground bg-muted/30 rounded-xl">
                  Select days above to configure their schedule
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={loading}
                  ...
                  ) : (editingRecurring ? "Save Changes" : "Save Schedule")}
                </button>
              </div>
            </form>
          </div>
